...
public class KMeans {
	 ...
	 * and finds the nearest one to the given record.
	 *
	 ...
	 * process, first we calculate the possible value range for each attribute. Then
	 * when we're going to generate the centroids, we generate random coordinates in
	...
	private static List<Centroid> randomCentroids(List<Record> records, int k) {
		...
		for (Record record : records) {
			record.getFeatures().forEach((key, value) -> {
				// compares the value with the current max and choose the bigger value between
				// them
				maxs.compute(key, (k1, max) -> max == null || value > max ? value : max);

				// compare the value with the current min and choose the smaller value between
				// them
